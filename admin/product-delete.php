  ...
  <tr>
   ...
   <?php
    if($_POST['user_action']=="delete" && $_POST['editKey'] != "" )
	{
		...
		$res = $obj->getAnyTableWhereData($obj->getTable("var_product_images"), " and product_id = '".$edit_key."' ") ;
		...
		$obj->my_query("delete from ".$obj->getTable("var_product_images")." where product_id = '".$edit_key."' ") ;
		//$obj->my_query("delete from ".$obj->getTable("var_product_pdf")." where product_id = '".$edit_key."' ") ;
		$obj->my_query("delete from ".$obj->getTable("var_product")." where  id = '".$edit_key."' ") ;
		
		$action = "product-manage.php";
		$msg_val = 'Product has been deleted successfully!';
